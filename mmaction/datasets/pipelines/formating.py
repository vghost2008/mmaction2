...
class FormatShapePose3D:
    ...
    def __call__(self, results):
        ...
        results['imgs'] = self.trans(imgs,results)
        if 'imgs_kp' in results:
            imgs = results['imgs_kp']
            results['imgs_kp'] = self.trans(imgs,results)
        results['input_shape'] = imgs.shape
        return results
    
    def trans(self,imgs,results):
        # [M x H x W x C]
        # M = 1 * N_crops * N_clips * L
        if self.collapse:
            assert results['num_clips'] == 1
