...
class UniformSampleFrames:
    ...
    def _get_test_clips(self, num_frames, clip_len):
        ...
        if num_frames < clip_len:
            ...
            for i in range(self.num_clips):
                offset = np.random.randint(bsize)
                all_inds.append(bst + np.maximum(0,offset+self.base_offset))
                if np.any(all_inds[-1]>=num_frames):
                    print(f"Error")
            inds = np.concatenate(all_inds)
        return inds
...
@PIPELINES.register_module()
class UniformSampleFramesV2:
    """
    In this version of UniformSampleFrames all inds almost have same distance #wjn
    Uniformly sample frames from the video.
    ...
    def _get_train_clips(self, num_frames, clip_len):
        ...
        assert self.num_clips == 1
        if num_frames < clip_len:
            delta = num_frames/clip_len
            inds = np.arange(clip_len)*delta
            ...
            bsize = np.diff(bids)
            max_v = np.max(bsize)
            offset = np.random.randint(max_v)
            offset = offset*np.ones_like(bsize)
            offset = np.minimum(offset,np.maximum(bsize-1,0))
            bst = bids[:clip_len]
            inds = bst + offset
        return inds
    ...
    def __call__(self, results):
        ...
        start_index = results['start_index']
        inds = inds + start_index+self.base_offset
        print("XXXXX:",num_frames,start_index,self.base_offset,inds,np.diff(inds),np.max(inds))
...
@PIPELINES.register_module()
class UniformSampleTailFrames:
    """Uniformly sample frames from the tail of video.

    To sample an n-frame clip from the tail of video. UniformSampleFrames basically
    divide the video into n segments of equal length and randomly sample one
    frame from each segment. To make the testing results reproducible, a
    random seed is set during testing, to make the sampling results
    deterministic.
    ...
    def __init__(self, clip_len, num_clips=1, test_mode=False, seed=255,base_offset=0,persent=0.25):
    ...
    def __call__(self, results):
        _num_frames = results['total_frames']
        if _num_frames*self.persent<self.clip_len:
            num_frames = min(_num_frames,self.clip_len)
            loffset = _num_frames-num_frames
        else:
            loffset = int(_num_frames*(1-self.persent))
            num_frames = _num_frames-loffset
        ...
        inds = np.mod(inds, num_frames)
        start_index = results['start_index']
        inds = inds + start_index+self.base_offset+loffset
...
class PoseDecode:
    ...
    def __call__(self, results):
        ...
        if 'keypoint_score' in results:
            #wjn
            #shape of keypoint_score is [person_nr,frames_nr,keypoint_nr]
        ...
        if 'keypoint' in results:
            #wjn
            #shape of keypoint is [person_nr,frames_nr,keypoint_nr,2] (x,y)
