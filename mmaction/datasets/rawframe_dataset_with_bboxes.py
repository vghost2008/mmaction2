...
class RawframeDatasetWithBBoxes(BaseDataset):
    ...
    def read_bboxes(path,total_frames):
        ...
        if not osp.exists(path):
            print(f"WARNING: file {path} dosen't exists, use default value.")
        ...
        with open(path) as f:
            while True:
                ...
                if len(line)<5:
                    ...
                    if len(infos) != int(line[0]):
                        print(f"ERROR bboxes data in {path}, expected {len(infos)} actual get {line[0]}")
                else:
                    datas = [float(x) for x in line[1:]]
                    datas = np.array(datas,dtype=np.float)
                    datas = np.reshape(datas,[-1,4])
                    minx = np.min(datas[:,0])
                    miny = np.min(datas[:,1])
                    maxx = np.max(datas[:,2])
                    maxy = np.max(datas[:,3])
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                bboxes_path = osp.join(self.data_prefix,frame_dir+"_bboxes.txt")
                ...
                if self.with_offset:
                    ...
                    idx += 1
                bboxes_info = self.read_bboxes(bboxes_path,total_frames=video_info['total_frames'])
                video_info['bboxes'] = bboxes_info
                # idx for label[s]
