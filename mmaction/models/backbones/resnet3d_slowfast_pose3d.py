...
class ResNet3dPathwayPose3D(ResNet3d):
    ...
    def __init__(self,
        ...
        if is_slow:
            lchannels = [[32,8],[64,8],[None,None]]
        else:
            lchannels = [[128,16],[256,16],[None,None]]
        self.cur_res_layer = 0
        self.lchannels = lchannels
        super().__init__(*args, **kwargs)
        self.inplanes = self.base_channels
        self.conv1_lateral = None
        ...
        for i in range(len(self.stage_blocks)):
            ...
            if i>=len(lchannels):
                continue
            tlchannels = lchannels[i]
            if tlchannels[0] is None:
                continue
    ...
    def make_res_layer(self,
        ...
        if self.cur_res_layer == 0:
            lateral_inplanes = 0
        elif self.lateral:
            if len(self.lchannels)<self.cur_res_layer:
                lateral_inplanes = 0
            else:
                lateral_inplanes = self.lchannels[self.cur_res_layer-1][1]
        else:
            lateral_inplanes = 0
        if lateral_inplanes is None:
            lateral_inplanes = 0
        self.cur_res_layer += 1
        if (spatial_stride != 1
...
pathway_cfg = {
    'resnet3d': ResNet3dPathwayPose3D,
...
class ResNet3dSlowFastPose3D(nn.Module):
    """Slowfast backbone.
    ...
    def __init__(self,
        ...
        self.slow_path = build_pathway(slow_pathway,is_slow=True)
        self.fast_path = build_pathway(fast_pathway,is_slow=False)
    ...
    def forward(self, x,imgs_kp=None):
        ...
        Returns:
            tuple[torch.Tensor]: The feature of the input samples extracted
                by the backbone.
        """
        x_slow = self.slow_path.conv1(imgs_kp)
        ...
        for i, layer_name in enumerate(self.slow_path.res_layers):
            ...
            if (i != len(self.slow_path.res_layers) - 1
                ...
                x_fast_lateral_shape = list(x_fast_lateral.shape)[-3:]
                x_fast_lateral_shape[-3] = 48
                x_fast_lateral = nn.functional.interpolate(x_fast_lateral,mode='nearest',size=x_fast_lateral_shape)
                x_slow = torch.cat((x_slow, x_fast_lateral), dim=1)
            if (i != len(self.slow_path.res_layers) - 1
                    and self.fast_path.lateral):
                lateral_name = self.fast_path.lateral_connections[i]
                conv_lateral = getattr(self.fast_path, lateral_name)
                #x_slow_lateral_shape = list(x_slow0.shape)
                #x_slow_lateral_shape[-3] = 8
                #x_slow0 = nn.functional.interpolate(x_slow0,mode='nearest',size=x_slow_lateral_shape)
                x_slow_lateral = conv_lateral(x_slow0)
                x_fast = torch.cat((x_fast, x_slow_lateral), dim=1)
...
if mmdet_imported:
    MMDET_BACKBONES.register_module()(ResNet3dSlowFastPose3D)
